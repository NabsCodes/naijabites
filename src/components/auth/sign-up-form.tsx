  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`HTTP ${res.status}: ${errorText || 'Failed to send OTP'}`);
      }
      ...
      // Handle development mode OTP display
      if (result.development && result.otp) {
        console.log("🔐 Development OTP:", result.otp);
        setError(`Development Mode: OTP is ${result.otp}. Email service unavailable.`);
      }
    } catch (err) {
