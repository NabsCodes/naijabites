...
export async function getShopifySaleProducts(
  limit: number = 8,
): Promise<Product[]> {
  const now = Date.now();
...
export async function getShopifyHomePageProducts(
  limit: number = 8,
): Promise<Product[]> {
  try {
...
export async function getShopifyProductBySlug(
  slug: string,
): Promise<Product | null> {
  try {
    const { getProductByHandle, transformShopifyProduct } = await import(
      "./shopify"
    );
